...
object TimeUsage extends TimeUsageInterface:
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) =
    ...
    val otherNeeds = Seq("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    columnNames.foldLeft((List.empty[Column], List.empty[Column], List.empty[Column])) {
      case ((primary, work, other), columnName) =>
        if primaryNeedsNames.exists(prefix => columnName.startsWith(prefix)) then (col(columnName) :: primary, work, other)
        else if columnName.startsWith("t05") || columnName.startsWith("t1805") then (primary, col(columnName) :: work, other)
        else if otherNeeds.exists(prefix => columnName.startsWith(prefix)) then (primary, work, col(columnName) :: other)
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column =
      when(
        col("telfs") > 0 && col("telfs") < 3, "working"
    ...
    // This function takes a list of columns and adds them, it then turns the minutes into hours and renames the column
    def sum(cols: List[Column], newName: String): Column = (cols.tail.foldLeft(cols.head)(_ + _) / 60).as(newName)

    val primaryNeedsProjection: Column = sum(primaryNeedsColumns, "primaryNeeds")
    val workProjection: Column = sum(workColumns, "work")
    val otherProjection: Column = sum(otherColumns, "other")
